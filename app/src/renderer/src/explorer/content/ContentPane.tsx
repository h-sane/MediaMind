...
import { PersonViewBanner } from './PersonViewBanner'
...
export function ContentPane(): React.JSX.Element {
  ...
  // Idempotent + cached, same call useDirectoryListing already makes for this
  // path — reusing it here just resolves the libraryId for the banner.
  ...
  return (
    <div
      ref={dropRef}
      className={`relative flex flex-1 flex-col overflow-hidden bg-white ${isOver ? 'ring-2 ring-inset ring-blue-400' : ''}`}
    >
      {personView && personLibrary.data && (
        <PersonViewBanner libraryId={personLibrary.data.id} personId={personView.personId} />
      )}
