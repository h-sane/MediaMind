        ...
        {file.persons.map((person: PersonOption) => {
          ...
          return (
            <button
              ...
              onClick={() => onChoose(file.file_id, isSelected ? null : person.person_id)}
...
/** Group-photo chooser — a file with more than one recognized person in
 * frame otherwise routes to whichever person has the most faces (F12); this
 * lets the user override that pick per file via `route_choices`. */
export function MultiPersonPanel({ libraryId, onBack }: Props): React.JSX.Element {
  ...
  return (
    <div className="h-full overflow-y-auto p-6">
      ...
      <div className="mb-6 flex items-end justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-500">
            These files contain faces from more than one person. Pick which person each file belongs to before
            organizing.
          </p>
        </div>
        {hasPendingChanges && (
          ...
          >
            {setChoices.isPending
              ? 'Saving…'
              : `Save ${Object.keys(pendingChanges).length} choice${Object.keys(pendingChanges).length === 1 ? '' : 's'}`}
          </button>
        )}
      </div>
      ...
      {isError && <p className="text-sm text-red-600">Could not load multi-person files. Run a face scan first.</p>}
      ...
      {!isLoading && files && files.length === 0 && (
        <div className="rounded-2xl border border-dashed border-zinc-300 py-16 text-center">
          ...
          <p className="mt-1 text-xs text-zinc-400">All files with face detections contain only one person.</p>
        </div>
      )}
